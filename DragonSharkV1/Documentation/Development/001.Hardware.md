...
2. A Wi-Fi adapter (compatibility was tested with `TL-WN725N Wi-Fi`).
3. A Bluetooth adapter (compatibility was tested with `UB500`). 
4. A sample display (compatibility was tested with `HMTECH 7" Display`, which is only 800x480; also, compatibility against higher resolution devices, like Smart TVs, was also tested).
5. A Micro SD card, and a USB adapter for Micro SD cards to be read in a desktop computer.
6. Gen Game New S3 bluetooth gamepads were also used.
