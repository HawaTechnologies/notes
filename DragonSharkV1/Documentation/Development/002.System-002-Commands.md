...
can be seen as additional ones inside `mmcblk0` (or whatever the eMMC disk is named):
...
1. Booting again from the SD card (it will allow to boot the system there).
2. Recognizing what mmcblkX is the disk, which will typically be unmounted and typically match the size
   of almost 16GB (be specially careful if your SD card is also 16GB in size). Do this with `lsblk` and
   inspect which disk matters for this purpose.
3. Then, the goal is to create (let's say in this example that `mmcblk0` is the disk - otherwise, it will
   plainly not allow you to do this) these partitions:
...
1. Limiting the current partition's size, in MB, from like ~14736 to 12477, this by reducing it in 2259MB
   (approximately 2.2gb that will MAKE use of later).
2. Creating a new partition, of format `ext4`, and 1.2GB..
3. Creating a new partition, of format `ext4`, and the remaining space. mounted at `/mnt/SAVES`.

The next step is setting the mount points for the partitions. This is done by:

1. Booting again from the eMMC system.
2. Listing the partitions with `lsblk -f`. The new ones will NOT be mounted.
3. Getting the UUIDs of the new partitions.
4. Editing the `/etc/fstab` to add the partitions:
...
...TAKE CARE OF THE UUIDs HERE...
UUID=1111-2222-3333-4444  /mnt/SAVES         ext4  defaults,noatime  0  2
UUID=5555-6666-7777-8888  /mnt/CURRENT_SAVE  ext4  defaults,noatime  0  2
```

And, then, again reboot. The next time, the partitions will be recognized.
...
Finally, for sanity purposes, let's set permissions and groups for those /mnt directories:

1. `pi` will need to write on /mnt/CURRENT_SAVE for web games, while `gamer` for native games.
   So the permissions here go 0777.
2. For /mnt/SAVES, it's OK for `pi` to be the only one to write it. So 0755 is OK to go.
...
The default password that comes for root with the supported image is: `Admin$2026`.
The default password that comes for pi with the supported image is: `Main$2026`.

For the `pi` user in particular, ensuring its auto-logged-in is mandatory:
...
And for some other tools we need, allow auto-sudo. This is done with `sudo visudo` and adding:
