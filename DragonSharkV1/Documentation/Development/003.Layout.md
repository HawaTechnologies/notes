...
20 consoles are supported for emulation, although the hardware may fail to properly make some of them work
(this problem exists with Nintendo 64 so far for few games - ongoing work is done to make it work). These
cores also include their .info files.

> An ongoing work is to ensure MAME emulation works properly for many games and formats. This ARMv8 hardware
  and system is undergoing a work with the 2000, 2003, 2003-plus, 2010 and "current" versions.
...
- Nintendo 64: `mupen64plus_libretro.so`.
- MAME: `mame_libretro.so` and variants: `mame2000_libretro.so`, `mame2003_libretro.so`,
        `mame2003_plus_libretro.so` and `mame2010_libretro.so`.
...
They're detailed [here](003.Layout-003-CoreConfigs.md).
...
Each directory, save for the `dragonshark` one, has two subdirectories: `saves` and `states`.
This stands for the two different save methods supported by RetroArch, but each core must define
whether it supports save either method or not.
...
Certain consoles need a setup of BIOS files. These ensure the per-region setup for a console is used.

Inside `ps1-bios/` there are BIOS files for PCSX=ReARMed emulator. Only one of them is needed. The chosen BIOS must
be moved, respecting its case-sensitive name, to `~/.config/retroarch/system` directory.

Inside `neogeo-bios/` there's a single `neogeo.zip` file that **must** be placed in the ROMs directory where the
Neo Geo games are stored. Contrary to the PlayStation bios files, this one is ignored if placed in the `system`
directory.

## Branding

The following images are being edited to fit the aesthetics of the console.

1. /boot/boot.bmp is replaced by [this image](boot.bmp).
2. /usr/share/plymouth/themes/armbian/watermark.png is replaced by [this image](watermark.png).
3. /usr/share/plymouth/themes/armbian/bgrt-fallback.png is replaced by [this image](bgrt-fallback.png).
4. /usr/share/backgrounds/dragonshark.jpg is [this image](background.jpg) and is set as wallpaper.

The previous images are kept in the same location but with an `_old` suffix to the base name.

The command `sudo update-initramfs -u -k all` was run so the images apply also on boot time.
